...
fn get_windows_directory(buffer: &mut [u8]) -> usize {
    unsafe {
        GetWidnowsDirectoryA(buffer.as_mut_ptr(), (buffer.len() - 1) as u32) as usize
    }
}

pub fn dll_is_known_dll(dll_name: &str) -> bool {
    let hklm = RegKey::predef(HKEY_LOCAL_MACHINE);
    let known_dlls = hklm.open_subkey("SYSTEM\\CurrentControlSet\\Control\\Session Manager\\KnownDLLs").expect("Could not find KnownDLLs subkey");
    let name_to_lower = dll_name.to_lowercase();
    for dll in known_dlls.enum_values() {
        match dll {
            Ok(name) => {
                if name.1.to_string().to_lowercase() == name_to_lower {
                    return true;
                }
            }
            Err(_) => continue,
